 ...
 * @returns 
 */
const getDelegationsAndRewardsHandler: RequestHandler<any, any> = async (
  req,
  res,
  next
): Promise<Response | void> => {
  try {
    const result = await ogmiosController.getDelegationsAndRewards(
      req.params.stakeKeyHashes
    );
